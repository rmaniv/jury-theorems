...
The jury.py script implements a Monte Carlo simulation to estimate the probability of reaching to a majority decision that is correct. The equivalent deterministic approach would do a summation of binomial probabilities for all combinations in which more voters are making the correct decision than not.
...
The `condorcet` function in a previous version of `theorems.py` that used the deterministic approach to estimate the probabilities produced the following graph for `n-values` of `1 3 12 100`:

![](https://github.com/vinamrsachdeva/jury-theorems/blob/main/condorcet.png)

Here's a graph for `n-values` of `4` that was generated by the Monte Carlo simulation:
